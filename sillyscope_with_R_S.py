...
class Sillyscope():
    ...
    name='TDS1012B'
        ...
        the default value is ugly with lots of "::"
    pyvisa_py=False
        Set to true if using pyvisa-py instead of, e.g., TekVISA or NI-VISA.
    """
    
    def __init__(self, name='TCPIP0::172.16.0.110::inst0::INSTR', pyvisa_py=True, simulation=False):
        ...
        try:
            ...
            try:
                ...
                # Identifying a Rohde&Schwarz scope and setting it as the model.
                elif idn[0:13] == "Rohde&Schwarz":
                    self.model = 'R&S'
    ...
    def trigger_single(self):
        ...
        elif self.model=='Rohde&Schwarz': self.write('RUN')
    ...
    def _query_and_decode_waveform(self):
        ...
        elif self.model == 'Rohde&Schwarz':
            width = 1
             
            # Get the data 
            try:
                self.write('FORM ASC')
                self.write('CHAN2:DATA?')
                s = self.read_raw()
                print(s)
...
if __name__ == '__main__':
    ...
    self = Sillyscope('TCPIP0::172.16.0.110::inst0::INSTR')
#    self = SillyscopeGUI() 
    
    resource_manager = _v.ResourceManager()
#    print(resource_manager.list_resources())
    #s = resource_manager.open_resource('TCPIP0::172.16.0.110::inst0::INSTR')
    #s.write('*IDN?')
    #print(s.read())
    self.set_channel(channel = 2) #??
    d = self.get_waveform(channel = 2)
    print(d)
    plt.plot(d[0], marker = 'o')
    
# To get scope data from console: 
# pulse = s.query('CHAN2:DAT?')
# pulse = pulse.split(',')
# dat = list(map(float, pulse))
# plt.plot(dat, marker = 'o', color = 'r')