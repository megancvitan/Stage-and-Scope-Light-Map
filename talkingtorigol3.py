...
#Takes amplitude of a DC signal.
def get_amp(input_name = 'None'):
    #Takes a screenshot and exports a .csv file at once.
    Rigol.write(":DISP:DATA?")
    bmpdata = Rigol.read_raw()[2+9:]
  
    #Save image file with a date and time stamp as the file name.
    ...
    #Stores information on which channels are being used.
    chanList = []
    ...
    #Will read only the displayed data when the scope is in RUN mode or when the MATH channel is selected.
    #Will read all the acquired data points when the scope is in STOP mode.
    Rigol.write(":MEASure:VMAX?")
...
#    print("Saved file:", "'" + filename)
#    print(csv_buff)
    scopedata = np.array(list(map(float, buff_list)))
    scopedata_mean = np.mean(scopedata)
    print('The average voltage measurement at this position is:', scopedata_mean, 'V.')
    Rigol.write("RUN")
    
    return scopedata_mean
...
def get_memory_depth(instr):
    ...
    if mdep == "AUTO\n": #If mdep is "AUTO"
        srate = instr.query(":ACQ:SRAT?") #ACQuire:SRATe
        scal = instr.query(":TIM:SCAL?") #TIMebase[:MAIN]:SCALe

        # mdep = h_grid * scal * srate
        mdep = h_grid * float(scal) * float(srate)

    return int(mdep) #Return mdep
